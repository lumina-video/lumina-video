...
mod rodio_impl {
    ...
    impl AudioPlayer {
        ...
        pub fn new_ring_buffer(
            ...
            let device_sample_rate = stream.config().sample_rate();
            tracing::info!("Audio device sample rate: {}Hz (ring buffer mode)", device_sample_rate);
            ...
            let (producer, consumer) = crate::media::audio_ring_buffer::audio_ring_buffer(ring_config);
    ...
    impl Source for RingBufferSource {
        #[inline]
        fn current_span_len(&self) -> Option<usize> {
            // None = no bounded span, rodio won't plan transitions
            None
        }
