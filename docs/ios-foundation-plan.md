...
poll_init() returns true + Ok
  → Ready (decoder + decode thread created)

poll_init() returns true + Err
...
23 tests in `crates/lumina-video-ios/tests/ffi_conformance.rs`. Tests NOT cfg-gated (run on all platforms via direct function import, not extern "C" linkage from macOS-only symbols).
